...
## When the Destination Is Not a Printing Press

This guide assumes you are going from a process build to a spot ink for commercial print. Two
related conversions come up often enough to be worth knowing about.

If the job has to stay in **process colour** — no fifth unit on press, or no budget for one —
converting to a spot colour only tells you what you cannot have. [Pantone to
Trumatch](/pantone-to-trumatch/) instead maps onto a systematic CMYK deck where every swatch is
already printable.

If the destination is **fabric rather than paper**, the CMYK you need is a different question
again: see [Pantone Textile to CMYK](/pantone-textile-to-cmyk/) for sublimation and DTG. And if
the colour is being manufactured rather than printed at all — painted, coated, dyed — start from
[Pantone to RAL](/pantone-to-ral/) or the [RAL to Pantone](/ral-to-pantone/) converter for the
reverse direction.

And once the spot colour is chosen, the job usually reverses: the printer wants the process build to
fall back on. The [Pantone to CMYK converter](/pantone-to-cmyk/) gives you that for any PMS number,
with the total area coverage of every build, a flag on the ones that leave the press no ink headroom,
and a section on rich black and total ink limits.
