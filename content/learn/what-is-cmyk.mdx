...
## CMYK Outside Commercial Print

Everything above describes CMYK on paper. The same four channels are used in two other places
where the rules differ enough to be worth flagging.

**Digital textile printing** — sublimation, direct-to-garment, pigment inkjet — accepts a CMYK
file but lays it onto fabric, and in sublimation the colour does not reach its final state until
heat has converted the dye. A build that is correct on paper is routinely wrong on cloth. See
[Pantone Textile to CMYK](/pantone-textile-to-cmyk/) for how that workflow differs.

**Systematic CMYK decks** take the opposite approach to Pantone: instead of cataloguing pre-mixed
inks, [Trumatch](/pantone-to-trumatch/) arranges around 2,000 colours on a regular grid where every
swatch is already a printable four-colour build. If a job has to print in process and you are still
choosing the colour, picking from a process deck avoids the disappointment entirely.
